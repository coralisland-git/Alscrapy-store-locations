...
class theisens(scrapy.Spider):
	name = 'theisens'
	domain = ''
	history = []

	def start_requests(self):
		init_url = 'http://www.theisens.com/LocationEventXML.aspx?zip=&miles=5000&locations=true&events=true&all=true&country=US'
	...
	def body(self, response):
		...
		for store in store_list:
			...
			item['store_type'] = self.validate(store.xpath('.//Type/text()'))
			yield item			
