...
class crowneplaza(scrapy.Spider):
	name = 'crowneplaza'
	domain = ''
	history = []

	def start_requests(self):
		init_url = 'https://www.ihg.com/crowneplaza/destinations/us/en/united-states-hotels'
		yield scrapy.Request(url=init_url, callback=self.parse_state)

	def parse_state(self, response):
		state_list = response.xpath('//div[contains(@class, "countryListingContainer")]//a/@href').extract()
		for state in state_list : 
			yield scrapy.Request(url=state, callback=self.parse_city)

	def parse_city(self, response):
		city_list = response.xpath('//div[contains(@class, "countryListingContainer")]//a/@href').extract()
	...
	def parse_page(self, response):
		try:
			...
			item['phone_number'] = self.eliminate_space(response.xpath('//div[@class="booking"]//text()').extract())[0]
			if item['store_name'] != '':
				yield item			
		except:
			pass
