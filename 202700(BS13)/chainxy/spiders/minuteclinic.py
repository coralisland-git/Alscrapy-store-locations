...
class minuteclinic(scrapy.Spider):
	...
	def parse_state(self, response):
		...
		for state in state_list : 
			state_link = (self.domain + state).split(';')[0]
	...
	def parse_city(self, response):
		...
		for city in city_list :
			city_link = (self.domain + city).split(';')[0]
	...
	def parse_store(self, response):
		...
		for store in store_list:
			store_link = (self.domain + store).split(';')[0]
	...
	def eliminate_space(self, items):
		tmp = []
		for item in items:
			if self.validate(item) != '' and 'set' not in self.validate(item).lower() and 'myClinic' not in self.validate(item):
				tmp.append(self.validate(item))
		return tmp
