...
class cititrends(scrapy.Spider):
	name = 'cititrends'
	...
	def parse_page(self, response):
		try:
			item = ChainItem()
			item['store_name'] = self.validate(response.xpath('//span[@class="location-name-brand"]/text()')) + ' ' + self.validate(response.xpath('//span[@class="location-name-geo"]/text()'))
			item['address'] = self.validate(response.xpath('//span[@class="c-address-street-1"]/text()'))
			item['city'] = self.validate(response.xpath('//span[@class="c-address-city"]//span/text()'))
			item['state'] = self.validate(response.xpath('//span[@class="c-address-state"]/text()'))
